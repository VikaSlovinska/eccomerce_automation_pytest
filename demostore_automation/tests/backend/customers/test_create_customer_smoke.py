...
pytest_mark = [pytest.mark.beregression, pytest.mark.besmoke, pytest.mark.customers_api]
...
def create_user_email_password_only():
    ...
    customer_api_info = CustomersAPIHelper().call_create_customer(email, password)
    customer_id = customer_api_info['id']

    data = {'email': email, 'password': password, 'api_response': customer_api_info}

    yield data

    # delete the test created customer
...
def test_create_customer_only_email_password():
    ...
    assert db_info[0]['display_name'] == expected_display_name, f"Display name database does not match expected." \
                                                                f"Email: {email}, " \
                                                                f"Expected display: {expected_display_name}" \
...
def test_create_customer_fail_for_existing_email():
    ...
    assert rs_body['code'] == 'registration-error-email-exists',\
        f"Create customer with existing user response does not" \
    ...
    assert 'An account is already registered with your email address.' in rs_body['message'], \
...
@pytest.mark.customers
@pytest.mark.tcid46
def test_autogenerated_username_from_email(create_user_email_password_only):
    logger.info("TEST: User name is autogenerated based on email")
    api_user_name = create_user_email_password_only['api_response']['user_name']
    email = create_user_email_password_only['email']
    expected_user_name = email.spli('@')[0]
    assert api_user_name == expected_user_name, f"Creating user with having only email and password should create" \
                                                f"user name based on email." \
                                                f"Expected user name: {expected_user_name}." \
                                                f" Actual user name: {api_user_name}"
